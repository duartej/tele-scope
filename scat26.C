...
//void scat26()
{
  ...
  cout << "try to open scat26.dat";
  ifstream D26( "scat26.dat" );
  if( !D26 ) {
    cout << ": failed" << endl;
    return;
  }
  ...
  while( D26.good() && ! D26.eof() ) {

    string rl;
    getline( D26, rl ); // read one line  = event into string
  ...
  TH1F * ht = new
    TH1F( "ht",
	  "scattering in Fe;Fe thickness [mm];RMS scattering angle [mrad]",
	  8, 0, 8 ); // axis range
  ht->SetStats(kFALSE); // no statistics
  ht->SetMinimum(0);
  ht->SetMaximum(8);
  ht->SetNdivisions( -802, "y" );
  ht->Draw();

  TGraphErrors * g48 = new TGraphErrors( n26, &vd[0], &v48[0], 0, &ve[0] );
  g48->SetMarkerColor(1);
  g48->SetMarkerStyle(20);
  g48->SetMarkerSize(1.2);

  TF1 * H48 = new
    TF1( "H48", "sqrt([0]*[0]+pow(13.6/4.80*sqrt(x/[1])*(1+0.038*log((x+0.001)/[1])),2))", 0, 8 );
  H48->SetNpx(500);
  H48->SetLineWidth(2);
  H48->SetParameter( 0, 0.2 );
  H48->SetParameter( 1, 18 ); // X0 [mm]
  H48->SetLineColor(kGray);
  //H48->Draw("same");

  cout << "Highland fit 4.8 GeV:" << endl;
  g48->Fit( "H48", "r" );
  g48->Draw("P"); // without axis option: overlay

  TGraphErrors * g24 = new TGraphErrors( n26, &vd[0], &v24[0], 0, &ve[0] );
  ...
  TGraphErrors * g12 = new TGraphErrors( n26, &vd[0], &v12[0], 0, &ve[0] );
  ...
  TLegend * lgnd = new TLegend( 0.20, 0.68, 0.43, 0.88 );
